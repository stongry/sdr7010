...
ax.barh(y, util_pct, color=["#1f77b4","#ff7f0e","#2ca02c","#d62728","#9467bd","#8c564b"])
for i, (u, t, p) in enumerate(zip(used, total, util_pct)):
    ax.text(min(p+1, 95), i, f"  {u:,} / {t:,}  ({p:.1f}%)", va="center", fontsize=10)
ax.set_yticks(y); ax.set_yticklabels(labels, fontsize=10)
ax.set_xlim(0, 110); ax.set_xlabel("utilisation %", fontsize=11)
ax.set_title("Digital OFDM+LDPC (build #34, pass_flag=1)\nfull bitstream resource usage on xc7z010clg400-2",
             fontsize=12, pad=10)
...
ax = axes[1]
res = designs["Path A pluto_ldsdr\n(ADI HDL pluto port,\nclg400 + LVDS_25)"]
labels = list(res.keys())
used = [v[0] for v in res.values()]
total = [v[1] for v in res.values()]
util_pct = [u/t*100 if t>0 else 0 for u,t in zip(used, total)]
...
ax = axes[2]
sizes = [3648, 1, 5378, 17600 - 5379 - 3649]   # LUT-RAM, LUT-SR, LUT-Logic, free
labels = ["LUT as Distributed RAM\n3,648", "LUT as Shift Reg\n1", "LUT as Combinational Logic\n5,378", "Free LUTs\n8,572"]
colors = ["#ff7f0e", "#d62728", "#1f77b4", "#dddddd"]
ax.pie(sizes, labels=labels, colors=colors, autopct="%1.1f%%", startangle=90,
       wedgeprops={"edgecolor":"white","linewidth":1})
ax.set_title("Digital design — LUT breakdown\n(total: 9,028 used / 17,600 available)",
             fontsize=12, pad=10)

# Pie chart: Path A LUT breakdown
ax = axes[3]
sizes = [120, 966, 11040, 17600 - 12127]
labels = ["LUT as DRAM\n120", "LUT as Shift Reg\n966", "LUT as Combinational Logic\n11,040", "Free LUTs\n5,473"]
colors = ["#ff7f0e", "#d62728", "#1f77b4", "#dddddd"]
ax.pie(sizes, labels=labels, colors=colors, autopct="%1.1f%%", startangle=90,
       wedgeprops={"edgecolor":"white","linewidth":1})
ax.set_title("Path A pluto_ldsdr — LUT breakdown\n(total: 12,127 used / 17,600 available)",
             fontsize=12, pad=10)
...
# Also generate the rtx wiring (block) diagram
